  ...
  function newContext(contextName) {
    ...
    function normalize(name, baseName, applyMap) {
      ...
      if (name && name.charAt(0) === '.') {
        ...
        if (baseName) {
          if (getOwn(_config.pkgs, baseName)) {
          ...
          pkgConfig = getOwn(_config.pkgs, pkgName = name[0]);
          name = name.join('/');
        ...
        if (mod.module) {
          ...
          return mod.module = {
            ...
            config: function config() {
              var c,
                  pkg = getOwn(_config.pkgs, mod.map.id); // For packages, only support config targeted
              // at the main module.

              c = pkg ? getOwn(_config.config, mod.map.id + '/' + pkg.main) : getOwn(_config.config, mod.map.id);
              return c || {};
            },
            exports: _defined[mod.map.id]
          };
        }
      }
    };
    ...
    context = {
      ...
      nameToUrl: function nameToUrl(moduleName, ext, skipExt) {
        var paths, pkgs, pkg, pkgPath, syms, i, parentModule, url, parentPath; //If a colon is in the URL, it indicates a protocol is used and it is just
